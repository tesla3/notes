...
### The Skill Market Is Real — But Not Where You'd Expect

An earlier version of this document concluded "the most valuable skill is the one you write yourself" and implied skills have no market beyond oneself. That was wrong — it conflated "most durable" with "most valuable," contradicting this document's own durability ≠ value principle (Section I). The actual market structure has four tiers:

| Category | What it encodes | Market scope | Durability | Examples |
|---|---|---|---|---|
| **Private knowledge** | Your deploy pipeline, oncall runbook, internal APIs | Self / team / company | Permanent | Bug pattern libraries, team convention codifiers |
| **Methodology seeds** | Opinionated process/workflow that others customize | Anyone with a similar workflow | High (opinions are durable) | `hn-distill` (editorial methodology), `commit` (commit conventions), research protocols, code review approaches |
| **Tool bridges** | Interface to a specific tool the model doesn't know well enough | Anyone using that tool | 6-18 months (until training absorbs it) | `agent-browser`, vendor skills (Firebase, Stripe, AWS) |
| **Staleness patches** | New framework patterns, updated API docs | Anyone on older models | Until next training cutoff | New React patterns, recently-changed API endpoints |

**Private knowledge skills have zero external market** — they encode what only you know, which is precisely why they're valuable to you and useless to anyone else. This is the permanent, durable tier, but it doesn't sell.

**Methodology seeds are the underrecognized market.** The `hn-distill` skill isn't private knowledge — it's an opinionated analysis methodology that works for anyone who distills HN threads. The `commit` skill encodes conventional commits, a public standard. Source credibility checks, research synthesis protocols, editorial constitutions — these are *curated process opinions* that are portable across users. The value isn't private knowledge; it's that someone else did the work of formalizing a methodology you'd otherwise have to figure out yourself. Think of them as recipes: the most valuable meal is the one you cook yourself, but cookbooks sell millions of copies because methodology seeds save the reader from reinventing the process. You download the seed, customize it, and it becomes your private knowledge over time.

**Tool bridges and staleness patches are the bulk of the marketplace** — the 57,000+ skills.sh listings are mostly here. This market is real, large, and churny. Vendors maintain their skills as developer relations. Communities maintain theirs until training data absorbs the patterns. The value is "save you the setup time now," not "give you durable competitive advantage." These aren't investments; they're consumables. Use them, benefit, expect to discard them.

**The marketplace isn't dead — it's structured differently than a typical software market.** skills.sh's 57K listings aren't waste. They're mostly consumables (tiers 3-4) with some methodology seeds (tier 2) and very little private knowledge (tier 1 — which shouldn't be published anyway). The value proposition is "save you setup time and give you a starting point to customize," not "give you a finished solution." The skills ecosystem inverts the usual software expectation: in most markets, the product ships finished and customization is optional. In skills, the *customization is the product* — the downloaded skill is just the seed.

**What this means for skill investment:**
- **Build** private knowledge skills (tier 1) — nobody else can, and they're permanent.
- **Publish** methodology seeds (tier 2) — they help others and survive model improvements.
- **Use** tool bridges and staleness patches (tiers 3-4) — don't build them unless you have to; let the community or vendor maintain them.
- **Don't confuse the tiers.** Using a downloaded tool bridge is fine. Treating a downloaded "security audit" skill as a substitute for expertise is the [Competence Illusion](#anti-pattern-1-the-competence-illusion).

**The format question:** If the value in tiers 1-2 is in the *practices* and not the *packaging*, then SKILL.md files are just one delivery mechanism among several — and not always the best one (see [Delivery Mechanism Fit](#delivery-mechanism-fit-not-everything-is-a-skill) above). AGENTS.md rules, extensions, CI config, project templates, and linter settings all deliver engineering practices to agents. The Agent Skills specification adds progressive disclosure and portability, which matter for some archetypes (tribal knowledge codification, session loops) and are irrelevant for others (verification gauntlets, constitutions). The framework in this document is about engineering practices that happen to benefit from agent awareness, not about a markdown file format. Use whatever mechanism makes the practice most reliable.
