...
- [Skills: Deep Synthesis — When, Why, ROI](research/skills-deep-synthesis-when-why-roi.md) — durability filter (durability ≠ value), 5 structural archetypes (gauntlet/loop/scaffold/constitution/codifier), delivery mechanism fit (skill vs hooks vs config vs AGENTS.md), ROI matrix (estimates, not data; maintenance cost explicit), anti-patterns, unbuilt skills (coding + non-coding), meta-insight (skills are engineering practices, format is secondary); includes [critical self-review](research/skills-deep-synthesis-critical-review.md) with post-revision update noting resolved/unresolved issues
