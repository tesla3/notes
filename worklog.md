...
## 2026-03-18 — Discord FTC archive pipeline review + user token risk research

- Critically reviewed `discord-ftc-archive-pipeline.md` — identified 15 gaps:
  - dotnet not installed (should be P0 not P1)
  - QMD version mismatch (doc references 1.1.0, installed is 2.0.1)
  - Channel/thread rename creates orphaned files (no ID-based directory mapping)
  - Embed/reaction rendering unspecified in converter scope
  - Requirement #3 (date/user filtering) not actually met by QMD alone
  - Several others documented in session
- Verified FTC Discord server: `discord.gg/ftc` resolves to "**Unofficial** FIRST® Tech Challenge Server" (doc incorrectly said "Official")
- Researched user token scraping risk extensively:
  - **March 2026 crackdown confirmed**: Discord actively detecting DiscordChatExporter via missing `x-super-properties` header
  - Found 4 first-person ban/restriction reports (2 from March 2026)
  - DCE PRs #1507/#1510 add the missing header — not yet merged
  - Piunikaweb article (March 12) + GitHub issue #1497 + Discussion #889 comment
- Evaluated all alternative archival tools (DHT, discord-fetch, DiscordChatExporterPy, pullcord, Discordless, discard2)
  - Self-audited analysis: corrected 7 errors (DHT detection mechanism, DHT .NET dependency, discord-fetch dead repo, conflating tool categories)
  - Key finding: every no-admin tool uses user token — no third path exists
  - DHT has better detection profile (requests go through real Discord client) but also needs .NET
  - discord-fetch GitHub repo is 404 — cannot recommend
- Verified Discord official data export only includes your own messages — useless for community archiving
- Created `notes/research/discord-user-token-risk-assessment.md` — full evidence-based risk assessment
- **Decision point**: bot token (ask admin, zero risk) vs user token (alt account, elevated risk). Binary choice.
- **Next steps**:
  - Check if DCE header fix PRs have been merged
  - Determine FTC server size (affects feasibility of user token approach)
  - Decide whether to ask FTC server admin for bot access
  - Update pipeline doc with corrections from review
