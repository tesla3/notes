...
- **[Distributed Systems Lessons for Parallel Agents](../research/distributed-systems-parallel-agents.md)** — CAP theorem analog, saga patterns, optimistic concurrency, semantic state gap (syntactic/semantic/process layers), NERDs entity-centered memory, why types+tests are the non-rotting semantic state layer
...
- **[Semantic Search Practitioner Playbook](../research/semantic-search-practitioner-playbook.md)** — actionable distillation: decision framework, tiered implementation stack, tool comparison (Augment/GrepAI/claude-context/Serena), cost reality check, what NOT to do (corrections incorporated: RTK input-vs-output token error, Augment single-repo caveat, Serena evidence gap)
